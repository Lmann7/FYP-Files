...
x = df.iloc[1:, in1:in2].values   # Input features (columns 2-3)
...
x_train, x_test, y_train, y_test =\
    train_test_split(x, y, test_size=split, random_state=RNDM)   # Splits data into training and testing sets
...
selected_columns = coef_df.columns[(abs(coef_df.iloc[0]) > 0.5) | (abs(coef_df.iloc[1]) > 0.5) | (abs(coef_df.iloc[2]) > 0.5)]
