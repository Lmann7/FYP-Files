...
# Filtering
df = df[(df['Type_x'] == 'Sa') | (df['Type_x'] == 'Sb') | (df['Type_x'] == 'Sc') | (df['Type_x'] == 'Sd')] 
...
row_indices = np.arange(0, 4, dtype=np.float64)
...
plt.xlabel("Diagonal (± offset)")  # Label considering positive and negative offsets
plt.ylabel("Classifications, Diagonal Sums")
plt.title("Histogram of Classifications")
...
combined_df.to_csv('Predictions-2.6-S3.csv', index=False)  # Save the dataframe to a csv file without row index
...
coef_df_sortedT.to_csv('coefficients-2.6-S3.csv', index=True)
flt=5
...
Highest_coef.to_csv('selected_measurables-2.6-S3.csv', index=False)
