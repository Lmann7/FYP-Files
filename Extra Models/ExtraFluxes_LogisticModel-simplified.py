...
df = pd.read_csv('allfluxes_in-simplified.csv')

# Prepare X and y for model training
x = df.iloc[1:, 2:17].values   # Input features (columns 2-3)
...
# For each row in the DataFrame (each model), create a separate bar chart
for i, row in coef_df.iterrows():
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.bar(x_pos, row, width=width)
    plt.title(f'Coefficients for Model {i+1}')
    plt.xlabel('Input variable')
    plt.ylabel('Coefficient')
    plt.xticks(x_pos, coef_df.columns, rotation='vertical')
    plt.show()
