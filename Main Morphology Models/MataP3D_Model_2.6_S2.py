...
# Filtering
df = df[(df['Type_x'] == 'Sa') | (df['Type_x'] == 'Sb') | (df['Type_x'] == 'Sab') | (df['Type_x'] == 'Sbc') | (df['Type_x'] == 'Sc') | (df['Type_x'] == 'Sd') | (df['Type_x'] == 'Scd')] 
...
le = LabelEncoder()
y = le.fit_transform(df.loc[0:, 'Type_x'].values)   # Target variable ('Classification' column)
...
# 1. Transpose the matrix to prepare for folding
matrix = cm
matrix_T = matrix.transpose()

#Triangular indexing for folding and summing
for i in range(7):
    for j in range(i+1, 7):
        matrix[i, j] += matrix_T[i, j]
        matrix_T[i, j] = 0  # Zero out the folded part for clarity
        
upper_diagonal_sums = []
for offset in range(0, 7):
    upper_diagonal_sums.append(np.diag(matrix, k=offset).sum())
print(matrix)

# Create row indices for x-axis
row_indices = np.arange(0, 7, dtype=np.float64)
# Adjust for centering the bars around their corresponding indices
#row_indices -= 0.5  # Shift indices to center bars 

# Create the histogram
plt.bar(row_indices, upper_diagonal_sums)
plt.xlabel("Diagonal (± offset)", fontsize=14)  # Label considering positive and negative offsets
plt.ylabel("Classifications, Diagonal Sums", fontsize=14)
plt.title("Histogram of Classifications", fontsize=18)
...
combined_df.to_csv('Predictions-2.6-S2.csv', index=False)  # Save the dataframe to a csv file without row index
...
coef_df_sortedT = coef_df_sorted.T

coef_df_sortedT.to_csv('coefficients-2.6-S2.csv', index=True)
flt=5

# Filter columns based on absolute coefficient values greater than 0.5
# Filter columns using a loop for the first 7 rows
selected_columns = coef_df_sorted.columns[
    (abs(coef_df_sorted.iloc[0]) > flt) |
    (abs(coef_df_sorted.iloc[1]) > flt) |
    (abs(coef_df_sorted.iloc[2]) > flt) |
    (abs(coef_df_sorted.iloc[3]) > flt) |
    (abs(coef_df_sorted.iloc[4]) > flt) |
    (abs(coef_df_sorted.iloc[5]) > flt) |
    (abs(coef_df_sorted.iloc[6]) > flt) 
]
Highest_coef = coef_df_sorted[selected_columns]

# Save the selected measurables and their coefficients to a CSV file
Highest_coef.to_csv('selected_measurables-2.6-S2.csv', index=False)
...
#feature_importance.to_csv('PermutationImportances-2.6-Sab-Scd.csv', index=False)

filtered_importance = feature_importance[feature_importance['Importance'] > .04]

ax = filtered_importance.plot(x='Feature', y='Importance', kind='barh', figsize=(7, len(filtered_importance)*.5), xerr='Standard Deviation', capsize=4)
ax.set_xlabel('Permutation Importance')
ax.set_title('Permutation Importance with Standard Deviation')
