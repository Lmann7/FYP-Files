...
combined_df.to_csv('Predictions-2.6-ELS.csv', index=False)  # Save the dataframe to a csv file without row index
...
coef_df_sortedT.to_csv('coefficients-2.6.csv', index=True)
flt=1.5
# Filter columns based on absolute coefficient values greater than 0.5
selected_columns = coef_df_sorted.columns[(abs(coef_df_sorted.iloc[0]) > flt) | (abs(coef_df_sorted.iloc[1]) > flt) | (abs(coef_df_sorted.iloc[2]) > flt)]

Highest_coef = coef_df_sorted[selected_columns]

# Save the selected measurables and their coefficients to a CSV file
Highest_coef.to_csv('selected_measurables-2.6.csv', index=False)
...
ax.legend(fontsize=15, bbox_to_anchor=bbox_to_anchor, loc=loc)
