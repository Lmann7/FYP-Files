...
df = pd.read_csv('Mata-P3D-3-imputed.csv')
...
coef_df_sorted.to_csv('coefficients-3-E-S.csv', index=True)

# Filter columns based on absolute coefficient values greater than 0.5
selected_columns = coef_df_sorted.columns[(abs(coef_df_sorted.iloc[0]) > 0.7)]

Highest_coef = coef_df[selected_columns]

# Save the selected measurables and their coefficients to a CSV file
Highest_coef.to_csv('selected_measurables-3-E-S.csv', index=False)
