...
df = pd.read_csv('Mata-P3D-2.6-S-imputed.csv')
# Filtering
df = df[(df['Type_x'] == 'S-a+b') | (df['Type_x'] == 'S-c+d')] 
...
#df.loc[df['Type_x'] == 'E', 'Type_x'] = 'TEMP'
#df.loc[df['Type_x'] == 'S', 'Type_x'] = 'E'
#df.loc[df['Type_x'] == 'TEMP', 'Type_x'] = 'S'
...
combined_df.to_csv('Predictions-2.6-Sab-Scd.csv', index=False)  # Save the dataframe to a csv file without row index
...
coef_df_sorted2.to_csv('coefficients-2.6-Sab-Scd.csv', index=True)

# Filter columns based on absolute coefficient values greater than 0.5
selected_columns = coef_df_sorted.columns[(abs(coef_df_sorted.iloc[0]) > 1.5)]

Highest_coef = coef_df_sorted[selected_columns]

# Save the selected measurables and their coefficients to a CSV file
Highest_coef.to_csv('selected_measurables-2.6-Sab-Scd.csv', index=False)
...
filtered_importance = feature_importance[feature_importance['Importance'] > .048]
