...
df = pd.read_csv('Multi_Input.csv')

# Replace '#DIV/0!' with NaN and convert columns to float starting from the second row
df.iloc[0:, 1] = df.iloc[0:, 1].replace('#DIV/0!', float('nan')).astype(float)
df.iloc[0:, 2] = df.iloc[0:, 2].replace('#DIV/0!', float('nan')).astype(float)

# Perform KL line calculation on the second column starting from the second row
df.loc[0:, 'KL_Calc'] = ((0.61/ ((df.iloc[0:, 1]) - 0.47)) + 1.19)   #=(0.61/(S2-0.47))+1.19

# Compares the calculated KL[log(O/Hb)] values with the actual log(O/Hb) values starting from the second row
df.loc[0:, 'comparison'] = df.loc[0:, 'KL_Calc'] < df.iloc[0:, 2]

# Replaces True/False with 'higher'/'lower' starting from the second row
df.loc[0:, 'comparison'] = df.loc[0:, 'comparison'].replace({True: 'higher', False: 'lower'})
...
for i in range(0, len(df)):
    ...
    elif 3 < df.iloc[i, 3] < 6:
        df.loc[i, 'Classification'] = 'Unclassified'
    else:
        df.loc[i, 'Classification'] = 'Passive'

# Save the output to a new CSV file
df.to_csv('multi_output.csv', index=False)
